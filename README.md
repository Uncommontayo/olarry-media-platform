...
3. Open the app in your browser (default: http://localhost:5174)
...
## Backend (Azure Functions)
The project includes a companion Azure Functions backend located in `./backend`. See `backend/README.md` for full details, including endpoint descriptions and local development steps.
...
For local backend development, consult `backend/README.md` (virtual environment, `STORAGE_CONNECTION_STRING`, and `func host start`).
...
## Development notes
- API surface (frontend ↔ backend): `POST /api/upload_media`, `GET /api/list_media`, `POST /api/like_media`, `DELETE /api/delete_media`, `GET /api/ai_caption` (see `backend/README.md`).
